...
export default function App() {
  ...
  const weeklyHours = (() => {
    ...
    let total = 0;
    for (let d = (weekNumber - 1) * 7; d < weekNumber * 7; d++) {
      const date = new Date(start);
      date.setDate(start.getDate() + d);
      const key = date.toISOString().split("T")[0];
      total += parseFloat((days[key] || {}).codingHours || 0);
    }
  ...
  function saveMeasurement() {
    setMeasurements(prev => ({ ...prev, [`week${measureWeek}`]: { ...measureForm, week: measureWeek } }));
    setMeasureForm({});
  }
  ...
  function saveNote() {
    ...
    if (editingNote !== null) {
      ...
      const newNote = {
        id: Date.now(),
        title: noteTitle || `Day ${dayNumber} note`,
        body: noteBody,
        tag: noteTag,
          ...
          <div onClick={() => setExpandedNote(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 16 }}>
            <div onClick={e => e.stopPropagation()} style={{ background: "#12121c", border: "1px solid #2a2a3a", borderRadius: 16, padding: 28, maxWidth: 500, width: "100%", maxHeight: "80vh", overflowY: "auto" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => startEdit(note, idx)} style={{ flex: 1, background: "#1e1e2e", border: "1px solid #2a2a3a", borderRadius: 8, padding: "10px", color: "#e8e8f0", fontSize: 12, cursor: "pointer", fontFamily: "'DM Mono',monospace" }}>
                  ✏️ Edit
                </button>
                <button onClick={() => deleteNote(note.id)} style={{ flex: 1, background: "#1e0d0d", border: "1px solid #3a1e1e", borderRadius: 8, padding: "10px", color: "#ff6363", fontSize: 12, cursor: "pointer", fontFamily: "'DM Mono',monospace" }}>
                  🗑 Delete
                </button>
              </div>
            </div>
      ...
      <div style={{ background: "#0e0e18", borderBottom: "1px solid #1e1e2e", padding: "20px 24px 0", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 2 }}>
            {tabs.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)} className="tbtn" style={{
                flex: 1, padding: "8px 1px", fontSize: 8, fontFamily: "'DM Mono',monospace",
                borderRadius: "8px 8px 0 0", background: tab === t.id ? "#12121c" : "transparent",
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
              {MEASUREMENT_FIELDS.map(f => (
                <div key={f.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#12121c", border: "1px solid #1e1e2e", borderRadius: 10, padding: "14px 16px" }}>
                  <div><div style={{ fontSize: 13 }}>{f.label}</div><div style={{ fontSize: 10, color: "#555", marginTop: 2 }}>{f.unit}</div></div>
          ...
          <div className="fi">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {NEETCODE_CATEGORIES.map(cat => {
                const done = neetcode[cat.id] || 0;
                const pct = (done / cat.total) * 100;
                const complete = done === cat.total;
                return (
          ...
          <div className="fi">
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <button onClick={() => { setComposing(true); setEditingNote(null); setNoteTitle(""); setNoteBody(""); setNoteTag("general"); }} style={{ background: "#63ffb4", color: "#0a0a0f", border: "none", borderRadius: 8, padding: "8px 14px", fontSize: 12, fontWeight: 700, fontFamily: "'Syne',sans-serif", cursor: "pointer" }}>
                  + New
                </button>
              </div>
            </div>
            ...
            {/* Compose / Edit area */}
            {composing && (
              <div style={{ background: "#12121c", border: "1px solid #2a2a3a", borderRadius: 14, padding: "20px", marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
                  {NOTE_TAGS.map(t => (
                    <button key={t.id} className="tag-pill" onClick={() => setNoteTag(t.id)} style={{
                      padding: "4px 12px", borderRadius: 20, fontSize: 11,
                      background: noteTag === t.id ? t.color + "33" : "#1e1e2e",
                      color: noteTag === t.id ? t.color : "#555",
                      border: `1px solid ${noteTag === t.id ? t.color : "#2a2a3a"}`,
                    }}>{t.label}</button>
                  ))}
                </div>
                ...
                <input
                  ...
                  placeholder={`Title (optional — defaults to "Day ${dayNumber} note")`}
                  style={{ ...inputStyle, marginBottom: 10, background: "#1a1a2a", border: "1px solid #2a2a3a" }}
                />
                <textarea
                  value={noteBody}
                  onChange={e => setNoteBody(e.target.value)}
                  placeholder="Write anything — thoughts, wins, struggles, ideas..."
                  rows={6}
                  style={{ ...inputStyle, resize: "vertical", lineHeight: 1.7, marginBottom: 12, background: "#1a1a2a", border: "1px solid #2a2a3a" }}
                />
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={saveNote} disabled={!noteBody.trim()} style={{ flex: 1, background: noteBody.trim() ? "#63ffb4" : "#1e1e2e", color: noteBody.trim() ? "#0a0a0f" : "#555", border: "none", borderRadius: 8, padding: 12, fontFamily: "'Syne',sans-serif", fontSize: 13, fontWeight: 800, cursor: noteBody.trim() ? "pointer" : "default" }}>
                    Save Note
                  </button>
                  <button onClick={() => { setComposing(false); setEditingNote(null); }} style={{ background: "#1e1e2e", border: "1px solid #2a2a3a", borderRadius: 8, padding: "12px 16px", color: "#888", fontSize: 12, cursor: "pointer", fontFamily: "'DM Mono',monospace" }}>
                    Cancel
                  </button>
                </div>
              </div>
            ...
            {notes.length > 0 && (
              <div style={{ marginBottom: 14 }}>
                <input
                  ...
                  style={{ ...inputStyle, background: "#12121c", border: "1px solid #1e1e2e" }}
                />
              </div>
            )}
                  ...
                  <div key={note.id} className="note-card" onClick={() => setExpandedNote(note.id)} style={{ background: "#12121c", border: "1px solid #1e1e2e", borderRadius: 12, padding: "16px 18px" }}>
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 8 }}>
                      <div style={{ flex: 1, marginRight: 12 }}>
                        ...
                        <div style={{ fontSize: 12, color: "#777", lineHeight: 1.6 }}>{preview}</div>
                      </div>
                    </div>
          ...
          <div className="fi">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 24 }}>
              {[
                ...
                const upcoming = !passed && (m.day - dayNumber) <= 7;
                return (
